...
export default function VendorWedyAIAssistant() {
  // Firma Özel Bağlamı (Context)
  const [userContext] = useState({
    role: 'firma',
    name: 'Beykoz Kır Bahçesi & Event',
    category: 'Düğün Mekanı / Kır Bahçesi',
    activeQuotes: '14 Açık Teklif',
    monthlyMeetings: '8 Randevu',
    conversionRate: '%68 Dönüşüm'
  });
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      id: '1',
      sender: 'ai',
      text: `Hoş geldiniz ${userContext.name}.\n\nWedyPlan Kurumsal B2B Asistanınız olarak hizmetinizdeyim. Açık teklifleriniz, müşteri iletişim diliniz ve sözleşme taslaklarınız konusunda size yardımcı olmaya hazırım.\n\nBugün ne üzerinde çalışmak istersiniz?`,
  ...
  const quickPrompts = [
    { title: 'VIP Teklif Metni', text: '300 kişilik hafta sonu kır düğünü için kurumsal ve ikna edici bir fiyat teklifi metni taslağı hazırla.', icon: FileText },
    { title: 'Müşteri İtiraz Yönetimi', text: 'Fiyatı yüksek bulan bir çifte nezaketle katma değerimizi anlatan profesyonel bir yanıt taslağı yaz.', icon: MessageSquare },
    { title: 'Sözleşme Maddesi', text: 'İptal ve kapora iade koşullarıyla ilgili hukuki açıdan koruyucu, şeffaf bir sözleşme maddesi oluştur.', icon: Briefcase },
  ];
    ...
    <div className="max-w-[900px] mx-auto space-y-8 pb-20 pt-4 font-sans selection:bg-[#111111] selection:text-white">
      ...
      <div className="text-center space-y-3">
        <div className="inline-flex items-center gap-2 px-4 py-1.5 bg-[#111111] text-white rounded-full text-[11px] font-medium tracking-widest uppercase shadow-sm">
          ...
          <span>WedyPlan Business • B2B Vendor Assistant</span>
        </div>
        
        <h1 className="text-[38px] md:text-[44px] font-serif font-normal tracking-tight text-[#111111] leading-tight">
          Kurumsal Firma & Satış Danışmanı
        </h1>
        
        <p className="text-[15px] text-[#666666] max-w-[600px] mx-auto font-light leading-relaxed">
          Tekliflerinizi hızlandıran, müşteri iletişimini optimize eden ve sözleşme süreçlerinizi yöneten B2B yapay zekanız.
        </p>
      </div>

      {/* Firma Dashboard Kartı */}
      <div className="bg-[#FBFBFB] border border-black/[0.06] rounded-[24px] p-5 flex flex-wrap items-center justify-between gap-4 shadow-sm">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="text-[12px] text-[#888888]">{userContext.category}</div>
          </div>
        </div>

        <div className="flex items-center gap-6 text-[13px] text-[#555555]">
          <div><span className="text-[#999999]">Açık Teklifler:</span> <strong className="text-[#111111] font-medium">{userContext.activeQuotes}</strong></div>
          <div><span className="text-[#999999]">Bu Ayki Randevular:</span> <strong className="text-[#111111] font-medium">{userContext.monthlyMeetings}</strong></div>
          <div><span className="text-[#999999]">Kapanış Oranı:</span> <strong className="text-[#111111] font-medium">{userContext.conversionRate}</strong></div>
        </div>
      </div>
      ...
      <div className="bg-white border border-black/[0.08] rounded-[32px] p-6 shadow-[0_20px_50px_rgba(0,0,0,0.03)] space-y-6 min-h-[480px] flex flex-col justify-between">
          ...
          {messages.map((m) => (
            ...
            >
              <div className={`w-9 h-9 rounded-full flex items-center justify-center text-xs shrink-0 shadow-sm ${
                ...
                {m.sender === 'user' ? <Building2 className="w-4 h-4" /> : <Sparkles className="w-4 h-4 text-[#D4AF37]" />}
              </div>
          ...
          {isLoading && (
            <div className="flex items-start gap-4 animate-pulse">
              ...
              <div className="p-5 rounded-[22px] rounded-tl-none bg-[#FBFBF9] border border-black/[0.05] text-[#111111] flex items-center gap-3">
                ...
                <span className="text-[13px] font-medium text-[#666666]">WedyPlan B2B Asistanı yanıt taslağı hazırlıyor...</span>
              </div>
            </div>
          )}
        ...
        >
          <input
            ...
            disabled={isLoading}
            placeholder="Örn: 'Fotoğraf paketi içinVIP teklif mesajı hazırla'..."
