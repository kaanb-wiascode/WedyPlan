...
export default function PremiumWedyAIAssistant() {
  ...
  return (
    <div className="p-6 sm:p-8 lg:p-10 max-w-[1000px] mx-auto space-y-8 font-sans">
      ...
      <div className="text-center space-y-2">
        <div className="inline-flex items-center gap-2 px-3 py-1 bg-rose-500/10 border border-rose-200/60 dark:border-rose-900/30 text-rose-600 dark:text-rose-400 rounded-full text-xs font-semibold uppercase tracking-wider">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {quickPrompts.map((p, idx) => (
          <button key={idx} disabled={isLoading} onClick={() => handleSend(p.text)} className="p-4 bg-white/80 dark:bg-zinc-900/60 border border-zinc-200/70 dark:border-zinc-800 rounded-2xl text-left hover:border-rose-300 dark:hover:border-rose-800 transition-all backdrop-blur-xl shadow-xs group cursor-pointer">
            <div className="flex items-center gap-2 text-zinc-900 dark:text-white mb-1.5">
              <p.icon className="w-4 h-4 text-rose-500" />
      ...
      {/* Chat Container */}
      <div className="bg-white/80 dark:bg-zinc-900/60 border border-zinc-200/70 dark:border-zinc-800 rounded-3xl p-6 backdrop-blur-xl shadow-xs min-h-[480px] flex flex-col justify-between space-y-4">
        ...
        <div className="space-y-6 overflow-y-auto max-h-[500px] pr-2 scrollbar-thin">
          {messages.map((m) => (
            <div key={m.id} className={`flex items-start gap-3.5 ${m.sender === 'user' ? 'flex-row-reverse' : ''}`}>
              <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 ${m.sender === 'user' ? 'bg-zinc-900 text-white dark:bg-white dark:text-zinc-900' : 'bg-rose-500 text-white'}`}>
                {m.sender === 'user' ? <User className="w-4 h-4" /> : <Sparkles className="w-4 h-4" />}
              </div>

              <div className={`p-4 rounded-2xl max-w-[82%] text-xs leading-relaxed ${m.sender === 'user' ? 'bg-zinc-900 text-white rounded-tr-none dark:bg-white dark:text-zinc-900' : 'bg-zinc-50 dark:bg-zinc-800/60 text-zinc-800 dark:text-zinc-200 rounded-tl-none border border-zinc-200/60 dark:border-zinc-700/60'}`}>
                ...
                {m.action?.type === 'THEME_GENERATED' && (
                  <div className="mt-3 p-4 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 rounded-xl shadow-xs">
                    <div className="flex items-center gap-2 mb-1.5">
                      ...
                      <h4 className="font-serif text-sm font-bold text-zinc-900 dark:text-white">{m.action.data.themeName}</h4>
                    </div>
        ...
        <form onSubmit={(e) => { e.preventDefault(); handleSend(); }} className="relative pt-2">
          <input type="text" value={input} onChange={(e) => setInput(e.target.value)} disabled={isLoading} placeholder="Örn: 'Kumsal düğünü için renk paleti öner'..." className={`w-full h-12 pl-4 pr-24 border rounded-2xl text-xs outline-none transition-all ${isListening ? 'bg-rose-50/50 border-rose-400' : 'bg-zinc-50/80 dark:bg-zinc-800/40 border-zinc-200/70 dark:border-zinc-700 focus:border-rose-500 focus:bg-white dark:focus:bg-zinc-900 text-zinc-900 dark:text-white'}`} />
          <div className="absolute right-2 top-1/2 -translate-y-1/2 flex gap-1.5">
            <button type="button" onClick={toggleListening} className={`w-8 h-8 rounded-xl flex items-center justify-center transition-colors ${isListening ? 'bg-rose-600 text-white animate-pulse' : 'bg-zinc-200/60 dark:bg-zinc-700 text-zinc-600 dark:text-zinc-300'}`}>
