  ...
  return (
    <div className="min-h-screen bg-[#FDFBF7] text-neutral-900 font-sans antialiased">
      ...
      <section className="pt-36 pb-20 px-4 max-w-5xl mx-auto text-center space-y-6">
        <span className="px-4 py-1.5 rounded-full bg-rose-50 border border-rose-200 text-xs font-semibold text-rose-700">
          🛠️ AKILLI DÜĞÜN PLANLAMA STÜDYOSU
        </span>
        <h1 className="text-4xl md:text-6xl font-serif tracking-tight">
          Masalsı Düğününüzü Dijitalde Tasarlayın
        </h1>
      ...
      <section className="pb-24 px-6 max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-6">
        {[
          { icon: "🌐", title: "Özel Düğün Web Sitesi", desc: "Davetlilerinize özel fotoğraflarınız, hikayeniz ve LCV formu içeren web sitesi oluşturun." },
          { icon: "📊", title: "Akıllı Bütçe Sihirbazı", desc: "Düğün harcamalarınızı kalem kalem takip edin, bütçenizi aşmadan plan yapın." },
          { icon: "🪑", title: "Masa & Oturma Mimarı", desc: "Davetli listelerinizi masalara sürükle-bırak yöntemiyle kolayca yerleştirin." },
          { icon: "💌", title: "Dijital Davetiye & LCV", desc: "WhatsApp ve sosyal medyadan gönderebileceğiniz interaktif LCV davetiyeleri." },
          { icon: "📋", title: "Adım Adım Kontrol Listesi", desc: "Aydan aya yapmanız gereken tüm görevleri zamanında tamamlayın." },
          { icon: "💼", title: "Sözleşme & Teklif Kasası", desc: "Firmalardan aldığınız teklifleri ve imzalanan sözleşmeleri tek yerde saklayın." },
        ].map((tool, idx) => (
