...
export default function VendorConsolePage() {
  ...
  return (
    <div className="flex min-h-screen bg-zinc-50/60 dark:bg-zinc-950 text-zinc-900 dark:text-zinc-100 font-sans">
      ...
      {/* SOL MENÜ (Sayfa Yenilenmeden Çalışan SPA Sidebar) */}
      ...
      <main className="flex-1 p-6 sm:p-8 lg:p-10 space-y-8 overflow-y-auto">
        ...
        {/* 1. DASHBOARD & GENEL BAKIŞ SEKMESİ */}
        {activeTab === 'overview' && (
          <div className="space-y-8 animate-in fade-in duration-300">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
              <GlassPanel className="p-5 space-y-2">
                ...
                <span className="text-[10px] text-zinc-500 font-medium block">Supabase Canlı Akış</span>
              </GlassPanel>
        ...
        {/* 2. TEKLİF TALEPLERİ SEKMESİ (overview ve leads için ortak gösterim) */}
        ...
        {/* 3. FİNANS SEKMESİ */}
        {activeTab === 'finance' && (
          <GlassPanel className="p-6 sm:p-8 space-y-4 animate-in fade-in duration-300">
            <h2 className="text-xl font-serif font-bold text-zinc-900 dark:text-white">Finans & Ciro Konsolu</h2>
            <p className="text-xs text-zinc-500 dark:text-zinc-400">Onaylanmış teklifler üzerinden toplam kazanç: <strong className="text-emerald-600">₺{(totalCiro * 0.88).toLocaleString('tr-TR')}</strong></p>
          </GlassPanel>
        )}

        {/* 4. CRM / İLETİŞİM SEKMESİ */}
        {activeTab === 'crm' && (
          <GlassPanel className="p-6 sm:p-8 space-y-4 animate-in fade-in duration-300">
            <h2 className="text-xl font-serif font-bold text-zinc-900 dark:text-white">İletişim & Müşteri CRM Konsolu</h2>
            <p className="text-xs text-zinc-500 dark:text-zinc-400">Gelen müşteri mesajları canlı olarak burada listelenmektedir.</p>
          </GlassPanel>
        )}
        ...
        {activeTab === 'calendar' && (
          <GlassPanel className="p-6 sm:p-8 space-y-4 animate-in fade-in duration-300">
            <h2 className="text-xl font-serif font-bold text-zinc-900 dark:text-white">Düğün & Etkinlik Takvimi</h2>
            <p className="text-xs text-zinc-500 dark:text-zinc-400">Rezervasyon tarihlerinizi ve doluluk oranlarınızı takip edin.</p>
          </GlassPanel>
        )}
        ...
        {activeTab === 'profile' && (
          <GlassPanel className="p-6 sm:p-8 space-y-4 animate-in fade-in duration-300">
            <h2 className="text-xl font-serif font-bold text-zinc-900 dark:text-white">Firma Profil Ayarları</h2>
            <p className="text-xs text-zinc-500 dark:text-zinc-400">Mekan görselleri, kapora oranları ve açıklama metinleri.</p>
          </GlassPanel>
        )}
        ...
        {activeTab === 'settings' && (
          <GlassPanel className="p-6 sm:p-8 space-y-4 animate-in fade-in duration-300">
            <h2 className="text-xl font-serif font-bold text-zinc-900 dark:text-white">Sistem & Bildirim Ayarları</h2>
            <p className="text-xs text-zinc-500 dark:text-zinc-400">E-posta ve SMS bildirim tercihleri.</p>
          </GlassPanel>
        )}
