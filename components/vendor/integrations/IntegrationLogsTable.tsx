...
export default function IntegrationLogsTable({ logs }: { logs: any[] }) {
  return (
    <div className="p-6 backdrop-blur-2xl bg-white/70 dark:bg-slate-900/70 border border-white/60 dark:border-slate-800 rounded-3xl shadow-sm overflow-x-auto space-y-4 text-xs">
      <div className="flex justify-between items-center">
        <span className="text-xs font-bold uppercase tracking-wider text-slate-400">
          📜 Canlı Senkronizasyon & API Log Geçmişi ({logs.length} Kayıt)
        </span>
      </div>
      ...
      <table className="w-full text-left border-collapse text-xs">
        <thead>
          <tr className="border-b border-slate-200/60 dark:border-slate-800 text-slate-400 font-semibold uppercase">
            <th className="py-3 px-2">Servis / Eylem</th>
            <th className="py-3 px-2">Zaman</th>
            <th className="py-3 px-2">Gecikme (Latency)</th>
          ...
          {logs.map((log) => (
            <tr key={log.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition">
              <td className="py-3 px-2 font-bold text-slate-800 dark:text-slate-100">
                {log.serviceName}
                <span className="block text-[10px] text-slate-400 font-mono font-normal">{log.action}</span>
              </td>
              <td className="py-3 px-2 text-slate-500">{log.timestamp}</td>
              <td className="py-3 px-2 font-mono text-slate-600 dark:text-slate-300">{log.latency}</td>
              ...
              <td className="py-3 px-2 text-right">
                <button
                  onClick={() => alert("📜 Payload Snapshot Modalı: " + log.id)}
