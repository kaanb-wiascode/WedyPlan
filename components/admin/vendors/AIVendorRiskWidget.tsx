...
export default function AIVendorRiskWidget({ aiReport }: { aiReport: any }) {
    ...
    <motion.div whileHover={{ y: -2 }} className="p-6 backdrop-blur-2xl bg-gradient-to-br from-indigo-500/10 via-white/80 to-purple-500/10 dark:from-indigo-950/30 dark:via-slate-900/80 dark:to-purple-950/20 border border-indigo-200/50 dark:border-indigo-900/40 rounded-3xl shadow-sm space-y-4 text-xs">
      <div className="flex justify-between items-center">
        ...
        <span className="px-2.5 py-1 rounded-full text-xs font-bold bg-emerald-100 text-emerald-700 dark:bg-emerald-950 dark:text-emerald-300">
          Kalite Skoru: %{aiReport.qualityScore}
        </span>
      </div>

      <div className="grid grid-cols-2 gap-2">
        <div className="p-3 rounded-2xl bg-white/80 dark:bg-slate-800/60 border border-indigo-100 dark:border-indigo-900/40">
          <span className="text-[10px] text-slate-400 font-bold uppercase block">🛡️ Risk Seviyesi</span>
          <span className="font-bold text-emerald-600 text-xs">%{aiReport.riskScore} (Düşük Risk)</span>
        </div>

        <div className="p-3 rounded-2xl bg-white/80 dark:bg-slate-800/60 border border-indigo-100 dark:border-indigo-900/40">
          <span className="text-[10px] text-slate-400 font-bold uppercase block">🚨 Şüpheli İşlem Bayrağı</span>
          <span className="font-bold text-slate-800 dark:text-slate-100 text-xs">{aiReport.fraudFlagsCount} İhlal</span>
        </div>
      </div>
      ...
      <div className="p-3.5 rounded-2xl bg-white/80 dark:bg-slate-800/60 border border-indigo-100 dark:border-indigo-900/40 space-y-1">
        <span className="text-[10px] text-slate-400 font-bold uppercase block">📝 AI Denetim Özeti</span>
      ...
      <div className="p-3 rounded-2xl bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-200/50 text-emerald-900 dark:text-emerald-200 font-semibold">
        💡 <strong>AI Rozet Tavsiyesi:</strong> {aiReport.recommendation}
      </div>
    </motion.div>
  );
}
