...
export default function ServiceCatalogBento({ services }: { services: any[] }) {
  if (!services || services.length === 0) return null;

  return (
    <div className="space-y-4 text-xs">
      <span className="text-xs font-bold uppercase tracking-wider text-slate-400 block">
        📚 Kurumsal Servis Kataloğu & Altyapı Bağımlılıkları (Service Catalog)
      </span>
            ...
            >
              <div className="flex justify-between items-start">
                <div>
                  ...
                  <span className="text-[10px] text-slate-400 font-mono">Takım: {item.team} • Kategori: {item.category}</span>
                </div>
              ...
              <div className="p-2.5 rounded-xl bg-slate-50 dark:bg-slate-800/50 border font-mono text-[10px] flex justify-between items-center">
                <span className="text-slate-400">Veritabanı / Domain:</span>
                <span className="font-bold text-indigo-600 text-xs">{item.db} ({item.domain})</span>
              </div>
            </motion.div>
          );
        })}
